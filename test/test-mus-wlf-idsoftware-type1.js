	...
	describe('I/O', function() {
		...
		describe('identify()', function() {
			...
			it('should reject short files', function() {
				...
				assert.equal(result.reason, 'File length 5 is less than minimum valid size of 6.');
				assert.equal(result.valid, false);
			});

			it('should reject lengths longer than content', function() {
				const result = handler.identify(
					content['truncated'].main,
					content['truncated'].main.filename
				);
				assert.equal(result.reason, `Content length 4 is shorter than length 15 given in header.`);
				assert.equal(result.valid, false);
			});
			...
			it('should reject lengths not multiples of 4', function() {
				...
				assert.equal(result.reason, `File length 7 is not a multiple of 4.`);
				assert.equal(result.valid, false);
			});
			...
			it('should reject non .wlf filename extension', function() {
				const result = handler.identify(
					content['bad_extension'].main,
					content['bad_extension'].main.filename
				);
				assert.equal(result.reason, `File extension ".imf" doesn't match required ".wlf".`);
				assert.equal(result.valid, false);
			});
